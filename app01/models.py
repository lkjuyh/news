...
class Type(models.Model):
    ...
    name = CharField(max_length=20, verbose_name="新闻类型")
...
class UserInfo(AbstractUser):
    ...
    image = ImageField(upload_to="image/%Y/%m", default='lu.gif', max_length=255, verbose_name="头像")
...
class Content(models.Model):
    ...
    type_id = ForeignKey(to="Type", to_field="id", related_name="content", verbose_name="新闻类型")
...
class Comment(models.Model):
    id = AutoField(primary_key=True)
    user_id = ForeignKey(to="UserInfo", to_field="id", related_name="comment", verbose_name="评论作者")
    news_id = ForeignKey(to="Content", to_field="id", related_name="comment", verbose_name="评论文章")
    ...
    state = BooleanField(default=True, verbose_name="审核状态")
    ip = GenericIPAddressField(verbose_name="用户IP地址")
